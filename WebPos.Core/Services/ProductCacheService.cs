...
public class ProductCacheService
{
    ...
    public async Task LoadCatalogAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (Product product in products)
        {
            ...
            {
                refreshedCatalogue[$"shortcode:{product.ShortCode}"] = product;
            }
        }
    ...
    public bool TryGetByShortCode(string shortCode, out Product? product)
    {
        ...
        return _catalogue.TryGetValue($"shortcode:{shortCode}", out product);
    }
