...
public sealed class PurchaseService : IPurchaseService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return _transactionService.ExecuteInTransactionAsync(async ct =>
        {
            ...
            foreach (PurchaseItem item in order.Items.OrderBy(i => i.Id))
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Product {item.ProductId} was not found for purchase receive.");
                }

                if (product.ParentProductId is not null)
                {
                    throw new InvalidOperationException(
                        $"Cannot receive stock into alias product '{product.Name}'. Receive against the master.");
                }

                int multiplier = UnitConversion.NormalizeMultiplier(product.ConversionMultiplier);
                ...
                long stockUnitCostPaisa = UnitConversion.ToStockUnitPricePaisa(
                    item.CostPricePerUnitPaisa,
                    multiplier);
                long stockUnitRetailPaisa = UnitConversion.ToStockUnitPricePaisa(
                    item.RetailPricePerUnitPaisa,
                    multiplier);

                decimal oldQty = product.StockQty;
                long oldCost = product.CostPricePaisa;
                decimal newQty = oldQty + stockQty;
                if (newQty > 0)
                {
                    product.CostPricePaisa = (long)Math.Round(
                        ((oldQty * oldCost) + (stockQty * stockUnitCostPaisa)) / newQty,
                        MidpointRounding.AwayFromZero);
                }
                else
                {
                    product.CostPricePaisa = stockUnitCostPaisa;
                }

                product.StockQty = newQty;
                if (stockUnitRetailPaisa > 0)
                {
                    product.RetailPricePaisa = stockUnitRetailPaisa;
                }

                product.UpdatedAt = now;

                // Legacy batch row kept for purchase-line linkage only (not inventory source of truth).
                ...
                ProductBatch batch = new()
                {
                    ...
                    PurchasePricePaisa = stockUnitCostPaisa,
                    RetailPricePaisa = stockUnitRetailPaisa,
                    InitialQty = stockQty,
                    CurrentQty = 0,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return _transactionService.ExecuteInTransactionAsync(async ct =>
        {
            ...
            foreach (QuickReceiveLineRequest line in request.Lines)
            {
                ...
                {
                    ProductBatch? lastBatch = await context.ProductBatches
                        .AsNoTracking()
    ...
    public Task<IReadOnlyList<PurchaseOrderSummaryDto>> ListAsync(
        int limit = 50,
        CancellationToken cancellationToken = default)
    {
        EnsureTenantResolved();
        ...
        {
            List<PurchaseOrder> orders = await context.PurchaseOrders
                .AsNoTracking()
            ...
            return (IReadOnlyList<PurchaseOrderSummaryDto>)orders.Select(o => new PurchaseOrderSummaryDto
            {
                Id = o.Id,
    ...
    public Task<PurchaseOrderDetailDto?> GetAsync(
        Guid purchaseOrderId,
        CancellationToken cancellationToken = default)
    {
        EnsureTenantResolved();
    ...
    public Task<PurchaseOrderDetailDto> UpdateOpenPurchaseAsync(
        Guid purchaseOrderId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return DbContextExecution.ExecuteAsync(_dbFactory, async (context, ct) =>
        {
            ...
            {
                await _partyService.GetSupplierAsync(supplierId, ct);
            ...
            foreach (CreatePurchaseLineRequest line in request.Lines)
            {
                ...
                bool productExists = await context.Products.AnyAsync(
            ...
            context.PurchaseItems.RemoveRange(existingItems);
            order.Items.Clear();
            await context.SaveChangesAsync(ct);
            ...
            return (await GetAsync(purchaseOrderId, ct))!;
        }, cancellationToken);
    }
