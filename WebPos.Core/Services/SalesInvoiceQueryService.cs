...
public sealed class SalesInvoiceQueryService(
    IDbContextFactory<WebPosDbContext> dbFactory,
    IAmbientDbContextAccessor ambient) : ISalesInvoiceQueryService
{
    ...
    public Task<IReadOnlyList<SalesInvoiceSummary>> ListByShiftAsync(
        Guid shiftId,
        int limit = 50,
        CancellationToken cancellationToken = default)
    {
        ...
        return DbContextExecution.ExecuteAsync(_dbFactory, async (context, ct) =>
        {
            List<SalesInvoice> invoices = await context.SalesInvoices
                ...
                .Take(take)
                .ToListAsync(ct);
            return (IReadOnlyList<SalesInvoiceSummary>)invoices.Select(ToSummary).ToList();
        }, cancellationToken);
    }

    public Task<IReadOnlyList<SalesInvoiceSummary>> ListRecentAsync(
        int limit = 50,
        CancellationToken cancellationToken = default)
    {
        int take = Math.Clamp(limit, 1, 200);
    ...
    public Task<IReadOnlyList<SalesInvoiceSummary>> SearchAsync(
        SalesInvoiceSearchQuery query,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Task.FromResult<IReadOnlyList<SalesInvoiceSummary>>([]);
        }
    ...
    public Task<SalesInvoiceDetail?> GetByInvoiceNoAsync(
        string invoiceNo,
        CancellationToken cancellationToken = default)
    {
        ...
        return DbContextExecution.ExecuteAsync(_dbFactory, async (context, ct) =>
        {
            ...
            List<SalesReturnItem> priorReturns = await (
                ...
                join sr in context.SalesReturns.AsNoTracking() on ri.SalesReturnId equals sr.Id
